...
class Gallery extends Widget
{
    ...
    public function renderItem($item)
    {
		...
		$options1 = ArrayHelper::getValue($item, 'options');
        $imageOptions = ArrayHelper::getValue($item, 'imageOptions', []);
        Html::addCssClass($options, 'gallery-item');
		Html::addCssClass($caption, 'col-sm-4');
		
		
		//$template = '<div class="caption"></div>';
		//$template1 = '<p>'.$options1.'</p>';

        return  Html::tag('div', Html::a(Html::img($src, $imageOptions), $url, $options). Html::tag('p', implode("\n", $options1), ['align' => 'justify']), $caption);
    }
