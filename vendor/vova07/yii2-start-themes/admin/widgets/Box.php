        ...
        if (!isset($this->buttons['batch-delete'])) {
            $this->buttons['batch-delete'] = [
                ...
                'options' => [
                    'id' => 'batch-delete',
                    'class' => 'btn-default',
                    'title' => Yii::t('vova07/themes/admin/widgets/box', 'Delete selected')
                ]
            ];
        }
        ...
        if (strpos($this->buttonsTemplate, '{batch-delete}') !== false && $this->grid !== null && isset($this->buttons['batch-delete'])) {
            ...
            $view->registerJs(
                "jQuery(document).on('click', '#batch-delete', function (evt) {" .
                    ...
                    "if (keys == '') {" .
                        "alert('" . Yii::t('vova07/themes/admin/widgets/box', 'You need to select at least one item.') . "');" .
                    "} else {" .
                        "if (confirm('" . Yii::t('vova07/themes/admin/widgets/box', 'Are you sure you want to delete selected items?') . "')) {" .
                            "jQuery.ajax({" .
                                "type: 'POST'," .
