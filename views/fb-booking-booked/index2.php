...
<div class="row">
  <div class="col-xs-3">
    <?php $form = ActiveForm::begin(['action' => Url::to(['booking/index']), 'method' => 'get']);
		  echo Html::dropDownList('uid', '', ArrayHelper::map(Profiles::find()->asArray()->all(), 'user_id', 'name'),['class'=>'form-control','prompt' => 'Select User', 'required' => 'required']);
  ...
  <div class="col-xs-12">
    <?php Box::begin(
		...
		$gridColumns = [
            ...
            'attribute' => 'user_id',
			'value' => 'profile.name',
			'filter' => Html::activeDropDownList($searchModel, 'user_id', ArrayHelper::map(Profiles::find()->asArray()->all(), 'user_id', 'name'),['class'=>'form-control','prompt' => 'Select User']),
			],
            //'facility_id',
			/*[
