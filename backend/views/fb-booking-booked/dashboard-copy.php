...
// echo $month_caption;
 ...
 $day_end = date("Y-m-" .$numDays);
	...
	//print_r($pending);
	$sql= "select * from fb_booking_booked where slot_from >= '$day_start' AND slot_to <= '$day_end' and status =1";
	$cmd =	Yii::$app->db->createCommand($sql);
	$query = $cmd->queryAll();
	$pending = count($query);
	
	$sql1= "select * from fb_booking_booked where slot_from >= '$day_start' AND slot_to <= '$day_end' and status =2";
	$cmd1 =	Yii::$app->db->createCommand($sql1);
	$query1 = $cmd1->queryAll();
	$confirm = count($query1);
	
	$sql2= "select * from fb_booking_booked where slot_from >= '$day_start' AND slot_to <= '$day_end' and status =3";
	$cmd2 =	Yii::$app->db->createCommand($sql2);
	$query2 = $cmd2->queryAll();
	$cancel = count($query2);
	
	$sql3= "select * from fb_booking_booked where slot_from >= '$day_start' AND slot_to <= '$day_end' and status =4";
